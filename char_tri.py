...
#******************************************************************************************************
#this function recieve char_ngrams_chunks directory path
#return accuracy in_domain and out_domain in the three tasks 
#******************************************************************************************************
def char_tri(dir_path):   
 ...
 for reddit_country in os.listdir(data_path1):
     ...
     and(reddit_country!="reddit.Mexico.txt.tok.clean")and(reddit_country!="reddit.NewZealand.txt.tok.clean")) :
         ...
         while(number_chunks<10000):  
           ...
           dirContents = os.listdir(join(country_path,author))
           if len(dirContents) == 0:
               author=np.random.choice(os.listdir(country_path))
               while(len(os.listdir(join(country_path,author)))==0):
                   author=np.random.choice(os.listdir(country_path))
           ...
           for l,line in enumerate(result):
             ...
             if line!="" and line !="\n" and line !=" \n":
                 i=0
                 while ((i<len(line)) and (i!=len(line))): #for every sentence extract char_trigrams
                     if i==len(line):
                         break
                     gram=""
                     L=9
 ...
 count_vect = CountVectorizer(tokenizer=lambda x: x.split(' '),max_features=1000)
...
def main(argv):
 start=process_time() 
 char_tri(str(sys.argv[1])) #char_ngrams_chunks directory 
 t=process_time() 
