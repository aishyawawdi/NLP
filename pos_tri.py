...
#******************************************************************************************************
#this function recieve pos_chunks directory path
#return accuracy in_domain and out_domain in the three tasks
#******************************************************************************************************
def pos_tri(dir_path):   
     ...
     and(reddit_country!="reddit.Mexico.txt.tok.clean")and(reddit_country!="reddit.NewZealand.txt.tok.clean")) :
         ...
         while(number_chunks<10000):  
           ...
           for l,line in enumerate(result):
             ...
             if line!="" and line !="\n" and line !=" \n":
                 sentence=line.split("), ")
                 for index,element in enumerate(sentence): #for every element in the sentence extract pos
                 ...
                 #for each sentence extract pos_trigrams
                 i=0
                 j=1
                 k=2
                 if len(sentence_pos)>=3:
                     sentence_tri.append("("+sentence_pos[i]+","+sentence_pos[j]+","+sentence_pos[k]+")")
                     while k!=(len(sentence_pos)-1):
                         i+=1
                         j+=1
                         k+=1  
                         sentence_tri.append("("+sentence_pos[i]+","+sentence_pos[j]+","+sentence_pos[k]+")")
 ...
 count_vect = CountVectorizer(tokenizer=lambda x: x.split(' '),max_features=300)
...
def main(argv):
 start=process_time()
 pos_tri(str(sys.argv[1])) #the path of pos_chunks directory
 t=process_time() 
